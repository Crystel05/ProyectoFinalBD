...
public class ControllerConsultas {
    ...
    public ArrayList<MejorCorredor> mejoresCorredoresGiro(Connection connection, int anno, String nombreGiro){
        ...
        try{
            ...
            ResultSet resultSet = callableStatement.executeQuery();
            while (resultSet.next()) {
                MejorCorredor corredor = new MejorCorredor(resultSet.getInt("PosicionFinal"), resultSet.getString("Nombre"), resultSet.getFloat("tiempoAcumulado")); //cambiar el último a int
    ...
    public static void main(String[] args){
        ControllerConsultas  con = new ControllerConsultas();
        ArrayList<MejorCorredor> corredors = con.corredoresMasRegulares(ControllerConexion.getInstance().connection, 2018, "Giro de Costa Rica");
        for(MejorCorredor i: corredors){
            i.imprimir();
        }
    }
}
